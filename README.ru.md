...
| **Маленькая** | 28 КБ кода при `-Os` со всем включённым, 15 КБ с `CI18N_MINIMAL`, 7,6 КБ на Cortex-M4. [Встраиваемые системы и размер](docs/ru/embedded-and-size.md) |
| **Проверенная** | Предупреждения как ошибки, санитайзеры и фаззинг, на Linux, macOS, Windows, BSD, iOS, Android, WebAssembly и голом железе ARM и RISC-V. [Платформы](docs/ru/platforms.md) |

Поиск занимает около 24 нс, а `gettext` из glibc около 133 нс на той же
