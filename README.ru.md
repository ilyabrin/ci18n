...
Направление письма для языков справа налево, разделители чисел по локали,
порядковые числительные и подключаемые форматтеры, чтобы плейсхолдер
`{created:date}` звал вашу функцию, а библиотека продолжала ничего не знать
о датах.
